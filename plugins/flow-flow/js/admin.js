...
;(function ( $ ) {
  ...
  var $errorPopup = $('<div id="error-popup"></div>');
	...
	$(document).one('html_ready ready', function () {
		...
		var templates = window.ff_templates; // defined in admin.php
...
//				console.log('nextId', nextId);
				...
				html = templates.view.replace(/%ROLES%/, rolesStr).replace(/%id%/g, nextId).replace(/%plugin_url%/g, plugin_url);
...
// Do something on success
          ...
          debugger
                    if (response.fb_extended_token == false){
                        ...
                        $fb_token.append('<p class="desc" style="margin: 10px 0 5px; color: red !important">! Extended token is not generated, Facebook feeds might not work</p>');
...
//			$streamsInput.val(JSON.stringify(streamsData));
    ...
    $content.on('mouseleave', '.shortcode-copy', function(){
      ...
      var html = templates.view.replace(/%ROLES%/, rolesStr).replace( /%id%/g, id).replace(/%plugin_url%/g, plugin_url);
