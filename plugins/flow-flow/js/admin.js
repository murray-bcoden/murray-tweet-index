...
;(function ( $ ) {
  ...
  var ALLOWED_NUMBER_OF_POSTS = 100;
  var MINUMUM_CACHE_LIFETIME = 3;
  ...
  $(document).one('html_ready ready', function () {
    ...
    function saveFeedsFrom ($cont, clone) {
      ...
      $cont.closest('.section-stream').find('input:text[id*="-posts"]').trigger('keyup', $views.length)
    ...
    $content.delegate('input[id*="-cache-lifetime"]', 'keyup', function(e) {
      var val = parseInt(this.value);

      if (!isNaN(val)) {
        if (val < MINUMUM_CACHE_LIFETIME
            && e.keyCode != 46 // delete
            && e.keyCode != 8 // backspace
        ) {
          e.preventDefault();
          this.value = MINUMUM_CACHE_LIFETIME;
          console.log('lifetime')
        }
      }
    })

    $content.delegate('input[id*="-posts"]', 'keyup', function(e, num) {
      var val = parseInt(this.value);
      var feed_num = num || $(this).closest('.section-stream').find('.feeds-list tbody tr').length

      if (!isNaN(val)) {
        if (val * feed_num > ALLOWED_NUMBER_OF_POSTS
            && e.keyCode != 46 // delete
            && e.keyCode != 8 // backspace
        ) {
          e.preventDefault();
          this.value = Math.floor(ALLOWED_NUMBER_OF_POSTS / feed_num);
          console.log('posts')
